...
resource "azurerm_subnet" "asav-outside" {
...
resource "azurerm_subnet" "asav-inside" {
...
resource "azurerm_route_table" "ASA_FW_RT_outside" {
  ...
  route {
    ...
    next_hop_in_ip_address = azurerm_network_interface.asav-outside.private_ip_address
  }
...
resource "azurerm_route_table" "ASA_FW_RT_inside" {
  ...
  route {
    ...
    next_hop_type  = "VirtualAppliance"
    next_hop_in_ip_address = azurerm_network_interface.asav-inside.private_ip_address
  }
...
resource "azurerm_subnet_route_table_association" "outside" {
  subnet_id                 = azurerm_subnet.asav-outside.id
  route_table_id            = azurerm_route_table.ASA_FW_RT_outside.id
}
resource "azurerm_subnet_route_table_association" "inside" {
  subnet_id                 = azurerm_subnet.asav-inside.id
  route_table_id            = azurerm_route_table.ASA_FW_RT_inside.id
}
...
resource "azurerm_virtual_machine" "asav-instance" {
  ...
  network_interface_ids = [azurerm_network_interface.asav-management.id,azurerm_network_interface.asav-outside.id,
                           azurerm_network_interface.asav-inside.id,azurerm_network_interface.asav-dmz.id]
  vm_size               = var.VMSize
