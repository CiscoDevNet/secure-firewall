...
resource "fmc_devices_bulk" "device1"{
...
data "fmc_devices" "device" {
  depends_on = [fmc_devices_bulk.device1]
  count = var.insCount
...
resource "fmc_staticIPv4_route" "route" {
  ...
  gateway {
    object {
      id   = fmc_host_objects.inside-gw[0].id
      type = fmc_host_objects.inside-gw[0].type
      name = fmc_host_objects.inside-gw[0].name
    }
  }
}
